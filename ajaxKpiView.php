...
if(isset($_POST["opid"])){
    ...
    // this query added for efficiency new calculation on 06.06.2021
        $query1New = "SELECT sum(a.PDQTY) AS QTY  FROM tpfn as a GROUP BY  a.LNNUM , a.BUDAT , a.TYPEF 
                      HAVING a.LNNUM='$opid' and a.BUDAT='$budat' and a.TYPEF='FIT' " ;
    ...
    $newfqty     = mysqli_query($db, $query6);
     ...
     // this query added for efficiency new calculation on 06.06.2021
             $qtyNew = mysqli_query($db, $query1New);
             if (mysqli_num_rows($qtyNew)>0) {
                 while($rowNew = mysqli_fetch_array($qtyNew)){
                    $data["ACTUALNEW"] = $rowNew["QTY"];
                 }
             } else {
                $data["ACTUALNEW"] = 1;
             }
            ...
            while($row5 = mysqli_fetch_array($neweff)){
                ...
                //eff calculation
                    ...
                    // $addeff =  $addeff + $lveff ; 
                     ...
                     // this query added for efficiency new calculation on 06.06.2021
                        $lvpmin = round( $row5["SMV"] * $data["ACTUALNEW"] ) ;
                        $lvsmin =  round ( $tminu * $row5["MP"] );
                        $lveff  =  round (( $lvpmin / $lvsmin ) * 100 );
                        $addeff =  $addeff + $lveff ; 
                     // End: this query added for efficiency new calculation on 06.06.2021
