...
<head>
    ...
    function onRemoveClosedOrder(){
        var person = prompt("Please enter your password");
        if ( person == '786') {
              var opid = 55;
              var soclose = document.getElementById('idsoclose').value;
                $.ajax({
                    type: 'POST', 
                    url: 'ajaxViewSetting.php',
                    data: {
...
<body class="w3-small" onload="onLoadDisplay()">
    ...
    <div class="w3-container">
        ...
        <div class="w3-bar w3-black">
            ...
            <button class="w3-bar-item w3-button tablink " onclick="openCity(event,'TABAD')">Remove Closed Order</button>
         </div>
        ...
        <!--END:Tab Div  Noman -->
         <div id="TABAD" class="w3-container w3-border city" style="display:none">
                <p>Remove closed order</p>
                <input type="text" name="soclose" id="idsoclose" placeholder="SO No" required>
                <button class="w3-button w3-teal" onclick="onRemoveClosedOrder()">Remove Closed Order</button>
         </div>
        <!--End:TABAD Noman-->
