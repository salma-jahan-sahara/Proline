<?php 
    include 'config.php';
    $sql = "SELECT id, RECIPIENT, LINENO, MESSAGE FROM line_issues ORDER BY id DESC LIMIT 1;";
    $result = mysqli_query($db, $sql);


    if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
            $line_issue_LINENO = $row["LINENO"];
            $line_issue_MESSAGE = $row["MESSAGE"];
            $line_issue_RECIPIENT = $row["RECIPIENT"];
        }
    }
?>
                ...
                <div class="col-4 border-end bg-dark text-light d-grid gap-2">
                    ...
                    <div class="row" style="background-color:rgb(24,24,24);">
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <h5 class="card-title text-danger ">LINE NO: <?php echo $line_issue_LINENO ?? ''; ?> </h5>
                                <p class="card-text "> <?php echo $line_issue_MESSAGE ?? ''; ?> </p>
                            </div>
                            <div class="card-footer text-muted">
                                Reported by: <?php echo $line_issue_RECIPIENT ?? ''; ?>
                            </div>
                        </div>
                    </div>
                </div>
